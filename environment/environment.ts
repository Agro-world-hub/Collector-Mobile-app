export const environment = {
    production: true,
    // API_BASE_URL: "https://api.dev.agroworld.lk/collector/agro-api/collection-api/",
    // API_BASE_URL: "https://dev.agroworld.lk/collector-api/agro-api/collection-api/",
    API_BASE_URL: "https://www.agroworld.lk/collector-api/agro-api/collection-api/",
    // API_BASE_URL: "http://192.168.8.102:3000/agro-api/collection-api/",
    SHOUTOUT_API_KEY:'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..ayaQjSjBxcSSnqskZp_F_NlrLa_98ddiOi1lfK8WrJ4'
  };
 