...
//       const response = await axios.get(
//         `${environment.API_BASE_URL}api/collection-manager/check-nic/${nic}`
//       );
...
//       setIsValidating(true);
//       const response = await axios.get(
//         `${environment.API_BASE_URL}api/collection-manager/check-phone/${phoneCode1}${phoneNumber}`
//       );
...
//       // Check NIC and phone number in parallel for efficiency
//       const [nicResponse, phoneResponse] = await Promise.all([
//         axios.get(`${environment.API_BASE_URL}api/collection-manager/check-nic/${formData.nicNumber}`),
//         axios.get(`${environment.API_BASE_URL}api/collection-manager/check-phone/${phoneCode1}${phoneNumber1}`)
//       ]);
      
//       if (nicResponse.data.exists) {
//         Alert.alert(t("Error.error"), t("Error.This NIC is already registered in the system."));
//         return;
//       }
      
//       if (phoneResponse.data.exists) {
//         Alert.alert(t("Error.error"), t("Error.This phone number is already registered in the system."));
//         return;
//       }
