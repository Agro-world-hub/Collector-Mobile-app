...
const SearchFarmer: React.FC<SearchFarmerProps> = ({ navigation }) => {
  ...
  const shouldShowNoResults = !isSearching && noResults && NICnumber.length > 0 && !ere;
  const shouldShowNewQr = !isSearching && newQr && farmers && NICnumber.length > 0;
      ...
      paddingRight: NICnumber.length === 0 ? 0 : 20, // Add padding when text is empty to ensure centering
          ...
          {shouldShowSearchImage && (
            <View className="mt-10 items-center">
              ...
              />
              {isSearching && (
                <Text style={[{ fontSize: 16 }, getTextStyle(selectedLanguage)]} className="text-center text-lg mt-4">{t("SearchFarmer.Searching")}</Text>
              )}
            </View>
          )}
