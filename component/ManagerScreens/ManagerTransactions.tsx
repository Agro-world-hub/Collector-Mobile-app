...
const ManagerTransactions: React.FC<ManagerTransactionsProps> = ({
  ...
  const handleSearch = (query: string) => {
    ...
    const filtered = transactions.filter((transaction) => {
      ...
      // const nicNumber = transaction.NICnumber || "";
       ...
       const nicNumberMatch = transaction.NICnumber
      ?.replace(/[^\w\s]/gi, '') 
      .toLowerCase()
      .includes(searchTerm);
 ...
 <TouchableOpacity onPress={() => navigation.goBack()} className="absolute mt-[4%] ">
            <AntDesign name="left" size={22} color="white" />
          </TouchableOpacity>
                    <Text className="text-white text-base font-bold ml-[35%] ">{t("ManagerDashboard.MyCollection")}</Text>
